module.exports = async (req, res) => {
  ...
  if (!key && !obj && !cid) return res.status(400).send("Missing key and objKey/clientId");

  // Resolve obj via latest if only clientId provided
  if (!obj && cid) {
    const r0 = await fetch(`https://${req.headers.host}/api/latest?clientId=${encodeURIComponent(cid)}`, {
...
<style>
  ...
  .dbg { color:var(--muted); font-size:12px; margin-top:10px }
</style>
...
<div class="dbg noprint"><small>Debug key: ${esc(obj)}</small></div>
`);
};
