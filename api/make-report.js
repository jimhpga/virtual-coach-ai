...
export default async function handler(req, res) {
  try {
    ...
    let status = {};
    if (await head(s3, statusKey)) status = await getJSON(s3, statusKey);
    ...
    res.status(200).json({ ok:true, bucket: BUCKET, jobId, reportKey, wrote_bytes: body.length, preview: report });
  } catch (e) {
    res.status(500).json({ ok:false, error: e?.message || String(e) });
  }
}

function keyToJobId(k){ return String(k||'').replace(/^uploads\//,'').replace(/\.[^.]+$/,''); }
async function head(s3, Key){ try{ await s3.send(new HeadObjectCommand({Bucket:BUCKET, Key})); return true; } catch { return false; } }
async function getJSON(s3, Key){
  const r = await s3.send(new GetObjectCommand({ Bucket: BUCKET, Key }));
  if (typeof r.Body?.transformToString === 'function') return JSON.parse(await r.Body.transformToString());
  const chunks=[]; for await (const ch of r.Body) chunks.push(ch);
  return JSON.parse(Buffer.concat(chunks).toString('utf8'));
}

// same sample as in analyze.js
