...
export default function ReportBetaClient() {
  ...
  (async () => {
    try {
      ...
      if (!rPose.ok || !rMap.ok) return;

      const pj = await rPose.json();
      const pm = await rMap.json();
      ...
      for (const f of raw) byFrame.set(Number(f.frame), f);

      const keys = ["P1","P2","P3","P4","P5","P6","P7","P8","P9"];
      const pframes = keys.map((k, idx) => {
        const fr = Number(p?.[k]?.frame ?? 0);
        const f = byFrame.get(fr) ?? raw[Math.max(0, Math.min(raw.length - 1, fr))] ?? {};
        return {
          p: idx + 1,
          frame: f.frame ?? fr,
          t_ms: f.t_ms ?? null,
      ...
      console.log("[VCA] Golden pose loaded:", { raw: raw.length, pframes: pframes.length, pmap: pframes.map(x=>x.frame) });
