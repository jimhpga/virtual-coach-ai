...
const FALLBACK: CardData = {
  swingScore: 82,
  tourDna: "Tour DNA Match: Player R (demo)",
  priority: "Face control + pressure shift",
  faults: [
    { title: "Face control: slightly open at P6", why: "Ball starts right / weak fade pattern under speed.", fix: "Keep lead wrist flatter through P5â€“P7; rotate, donâ€™t flip." },
    { title: "Pressure shift late (hanging back)", why: "Low point drifts back; contact gets thin/heel-y.", fix: "Shift to lead side by P5, then rotate around a posted lead leg." }
  ],
  drills: [
    { name: "P6 Check + Turn Through", steps: ["Stop at P6 (shaft parallel downswing).","Lead wrist flat, logo of glove slightly down.","Turn chest to target without adding hand flip."], reps: "3 sets of 6 slow reps, then 3 full swings" },
    { name: "Step-Through Pressure Shift", steps: ["Start feet together, take club to P2.","Step lead foot to target as arms reach P3â€“P4.","Swing through to a tall, balanced finish."], reps: "2 sets of 8, then 5 normal swings" }
  ]
};
...
export default function CardClient() {
  const [data, setData] = React.useState<CardData>(FALLBACK);

  React.useEffect(() => {
  cardSrc = "/data/card-demo.json";
  try {
    const qp = new URLSearchParams(window.location.search);
    const qsrc = qp.get("src");
    if (qsrc) cardSrc = qsrc;
    else {
      const ss = sessionStorage.getItem("vca_card_src");
      if (ss) cardSrc = ss;
    }
  } catch {}// Data source priority:
    // 1) ?src=/data/xyz.json  (explicit)
    // 2) sessionStorage vca_card_src
    // 3) /data/card-demo.json
    // Always fallback to FALLBACK.
    ...
    try {
      ...
      if (q && q.startsWith("/")) cardSrc = q;

      const ss = window.sessionStorage.getItem("vca_card_src");
      if (!q && ss && ss.startsWith("/")) cardSrc = ss;
    } catch (e) {}

    fetch(cardSrc, { cache: "no-store" })
      .then((r) => (r.ok ? r.json() : null))
      .then((j) => { if (j) setData(j as CardData); })
      .catch(() => {});
  }, []);const swingScore = data.swingScore;
  const tourDna = data.tourDna;
  const topFaults = data.faults;
      ...
      <div style={{
        ...
        <div style={{
          ...
          <div style={{ display: "grid", gap: 6 }}>
            <div style={{ fontSize: 14, opacity: 0.85 }}>Virtual Coach AI â€” Report Card (Demo)</div>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "380px 1fr", gap: 14, padding: 18 }}>
          <div style={{
            ...
            <div style={{ display: "grid", gap: 8 }}>
              <div style={{ fontSize: 12, opacity: 0.75 }}>Todayâ€™s Priority</div>
              <div style={{
                padding: 12,
                borderRadius: 14,
                border: "1px solid rgba(255,255,255,0.12)",
