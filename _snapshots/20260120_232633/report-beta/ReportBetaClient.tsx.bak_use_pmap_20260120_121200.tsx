...
const [runStatus, setRunStatus] = React.useState<string>("");
  ...
  // ✅ If /upload-live sent us an uploaded video URL, use it (real-product behavior).
  React.useEffect(() => {
    try {
...
useEffect(() => {
  // Golden demo: hydrate overlay frames from /demo/pose.json (served from /public/demo/pose.json)
  if (typeof window === "undefined") return;
  const params = new URLSearchParams(window.location.search);
  const golden = params.get("golden") === "1";
  if (!golden) return;

  const pick = (arr: any[], t: number) => {
    if (!arr?.length) return null;
    const i = Math.max(0, Math.min(arr.length - 1, Math.round(t * (arr.length - 1))));
    return arr[i];
  };

  (async () => {
    try {
      const r = await fetch("/demo/pose.json", { cache: "no-store" });
      if (!r.ok) return;
      const pj = await r.json();
      const raw = pj?.frames ?? [];
      if (!raw?.length) return;

      // Build P1..P9 frames by sampling evenly across the clip
      const ts = [0.00, 0.12, 0.24, 0.36, 0.50, 0.62, 0.74, 0.86, 0.98];
      const pframes = ts.map((t, idx) => {
        const f = pick(raw, t) || {};
        return {
          p: idx + 1,
          frame: f.frame ?? null,
      ...
      console.log("[VCA] Golden pose loaded:", { raw: raw.length, pframes: pframes.length });
  ...
  <div style={{ fontSize: 12, letterSpacing: 1, opacity: 0.7, marginBottom: 6 }}>
    PLAYER OVERVIEW
          ...
          <div style={{ marginTop: 6, fontSize: 13 }}>
            <b>Today’s ONE priority:</b> {post.priorityLabel}
          </div>
    ...
    <div style={{ display: "grid", gap: 8 }}>
      <Collapsible title="Ask the Coach (optional)" defaultOpen={false}>
        <div style={{ display: "grid", gridTemplateColumns: "1fr auto", gap: 10, alignItems: "end", marginTop: 8 }}>
          ...
          <button
            type="button"
            onClick={() => alert("ðŸŽ¤ Voice is coming soon. For now, type your question.")}
            style={{
              height: 42,
              padding: "0 12px",
              borderRadius: 12,
