...
async function main() {
  ...
  try {
    ...
    const metaMap = {
      ...
      // Back-compat with older fallback keys you used earlier
      face_control: { label: "Clubface control (impact)", meaning: "Face-to-path is drifting — start line control suffers.", drills: ["9-to-3 punch shots: half swings, hold the face square through impact.","Alignment stick gate: start the ball through a narrow start-line window."] },
      low_point: { label: "Low point control (strike)", meaning: "Bottom of arc is inconsistent — contact and compression vary.", drills: ["Towel drill: place towel 4–6\" behind ball; miss towel, hit ball first.","Line drill: draw a line, ball just ahead; strike the line in front every rep."] },
      sequence: { label: "Sequence timing (transition)", meaning: "Body/arms aren’t syncing — speed leaks and face gets flippy.", drills: ["Step-through drill: small step toward target to trigger pressure shift then turn.","Pump drill (3 pumps): rehearse P5 slot, then swing through without flipping."] }
    };
    ...
    if (needsTop || needsRanked) {
      ...
      rep.rankedFaults = base.map((k, i) => {
        const m = metaMap[k] || {
          label: String(k).replace(/_/g, " "),
          meaning: "Demo fallback.",
          drills: ["Slow reps to a checkpoint, then swing.", "Film 2 swings and compare positions."]
        };
        return {
          key: k,
          score: 60 - i * 5,
    ...
    if (!Array.isArray(rep.evidence) || rep.evidence.length === 0) {
      const rf = Array.isArray(rep.rankedFaults) ? rep.rankedFaults : [];
      rep.evidence = rf.map(x => ({
        faultKey: x.key,
        why: x.meaning || "Demo fallback (scores not available)",
        source: "fallback"
      }));
    }
