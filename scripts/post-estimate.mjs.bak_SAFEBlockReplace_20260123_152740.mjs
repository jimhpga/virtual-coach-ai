...
async function main() {
  ...
  try {
    if (!Array.isArray(rep.topFaults) || rep.topFaults.length === 0) {
      const k1 = rep?.priorityKey || "face_control";
      const k2 = "low_point";
      const k3 = "sequence";

      rep.topFaults = [k1, k2, k3].filter(Boolean).slice(0, 3);
      rep.rankedFaults = rep.topFaults.map((k, i) => ({
        key: k,
        score: 60 - i * 5,
        label: String(k).replace(/_/g, " "),
        note: "Demo fallback"
      }));
      rep.confidence = rep.confidence || { value: 0.55, sep: 0 };
      rep.evidence = rep.evidence || rep.topFaults.map(k => ({
        faultKey: k,
