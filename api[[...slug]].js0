// /api/[[...slug]].js
// ONE serverless function that handles all routes under /api/*
// Works on Vercel Hobby (avoids the “>12 functions” limit)
...
export const config = {
  api: { bodyParser: false } // we handle JSON + multipart ourselves
};
...
function qcReport(report = {}) {
  ...
  // sample fields (we handle if they exist)
...
async function routeUpload(req, res) {
  // Guard: Vercel Hobby will 413 anything big; this path is for tiny files only.
...
// GET /api/analyze?key=...
// Stub: if <key>.report.json exists, return it; else pending.
...
// GET /api/qc?key=...
// Runs simple sanity checks over the report (if present)
