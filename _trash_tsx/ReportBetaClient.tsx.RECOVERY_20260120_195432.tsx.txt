...
export default function ReportBetaClient() {
  ...
  // Step label helper (prevents "stepName is not defined")
  const stepName = (s: any) => {
    const v = String(s ?? "");
    if (v === "upload" || v === "Upload") return "Upload";
    if (v === "analyze" || v === "Analyze") return "Analyze";
    if (v === "report" || v === "Report") return "Report";
    if (s && typeof s === "object" && "step" in s) return String((s as any).step);
    return v || "Upload";
  };
