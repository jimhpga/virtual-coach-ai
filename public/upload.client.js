(() => {
  ...
  fileInput?.addEventListener("change", (e) => {
    ...
    fileLabel && (fileLabel.textContent = f ? `${f.name} (${f.type || "video"}), ${f.size} bytes` : "(no file)");
  ...
  btn?.addEventListener("click", async () => {
    ...
    try {
      ...
      log("If this says 404/NOT_FOUND, ensure these /public files reached MAIN so Vercel deploys them.");
      busy(false);
    }
  });
})();
