...
export default function UploadPage() {
  ...
  // Keep a local blob preview ONLY for this page.
  useEffect(() => {
    if (!file) return;
  ...
  const shell: React.CSSProperties = {
    minHeight: "100vh",
    background:
      "radial-gradient(1200px 800px at 20% 10%, rgba(34,197,94,0.12), transparent 60%), radial-gradient(900px 600px at 90% 20%, rgba(56,189,248,0.10), transparent 55%), #050b16",
    color: "#e6edf6",
    padding: "44px 22px",
  };
  ...
  async function onSubmit() {
    ...
    // MVP: store base64 so /report can recreate a fresh blob there (no MIME/expired blob issues)
    try {
      const base64 = await fileToBase64(file);
      sessionStorage.setItem("vca_video_base64", base64);
    } catch (e) {
      console.error("Failed to store preview video in sessionStorage", e);
    }
