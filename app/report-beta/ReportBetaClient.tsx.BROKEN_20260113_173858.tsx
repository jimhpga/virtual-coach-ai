...
export default function ReportBetaClient() {
  ...
  const [swingFacts, setSwingFacts] = React.useState<SwingFacts | null>(null);

    // ===== VCA_INTAKE_SAFE_USEEFFECT =====
  // Prevent client crash if storage is blocked / unavailable
  useEffect(() => {
    try {
      const s = sessionStorage.getItem("vca_intake");
      if (!s) return;
      const obj = JSON.parse(s);
      // stash for UI if you want it later
      // setIntake(obj);
    } catch (e) {
      // ignore
    }
...
React.useEffect(() => {
    ...
    fetch("/pose-demo/swing_facts.json", { cache: "no-store" })
      ...
      .then(j => { if(alive && j) setSwingFacts(j); })
      .catch(() => {});
    return () => { alive = false; };
  }, []);

  function buildPrioritySummary(args: {
    focus?: string;
    facts?: SwingFacts | null;
    topFaults?: string[];
  }){
    const vcaFocus = (args.focus || "").trim();
    const focus = vcaFocus;
    const conf = args.facts?.confidence || "—";
    const det  = (args.facts?.detectedRatePct ?? null);
    const tempo = args.facts?.tempo?.ratio ?? null;
    const vcaFaults = (args.topFaults || []).filter(Boolean).slice(0,2);

    // This is intentionally "coach voice": define situation + next actions.
    const parts: string[] = [];

    if(det != null){
      parts.push(`We detected your body reliably on ${det}% of frames (${conf} confidence), so this report is based on real pose tracking — not vibes.`);
    } else {
      parts.push(`This report is based on your upload + checkpoints. Some parts may be estimated until tracking confidence is higher.`);
    }

    if(focus){
      parts.push(`Your stated focus: “${focus}.”`);
    }

    if(faults.length){
      parts.push(`Priority today: fix ${faults.join(" + ")} first — everything else is noise.`);
    } else {
      parts.push(`Priority today: clean up one thing at a time. Start with setup + first move, then retest.`);
    }

    if(tempo != null){
      parts.push(`Tempo ratio (backswing ÷ downswing) is about ${tempo}. Don’t chase speed — chase sequence.`);
    }

    return parts.join(" ");
  }
  // ===== VCA_PRIORITY_BANNER_END =====
// ===== VCA_COACH_EXPLAIN_START =====
  // This is not a "summary" — it's the lesson. It can be long if it needs to be.
  function buildCoachExplanation(args: {
    focusText: string;
    faults: FaultKey[];
    rx: any[];
    overview?: string;
  }): string[] {
    const vcaFocus2 = (args.focusText || "").trim();
    const vcaFaults2 = Array.isArray(args.faults) ? args.faults : [];
    const rx = Array.isArray(args.rx) ? args.rx : [];
    const overview = (args.overview || "").trim();

    const primary = rx[0]?.drill;
    const primaryTitle = primary?.title || "Primary Fix";
    const primaryWhy = primary?.why || "";
    const primaryHow: string[] = Array.isArray(primary?.how) ? primary.how : [];
    const primaryCue = primary?.onCourseCue || "";

    // Convert faults into plain-English observations (best-effort)
    const obs: string[] = [];
    const add = (s: string) => { if(s && !obs.includes(s)) obs.push(s); };

    faults.forEach((f: string) => {
      const k = (f || "").toLowerCase();
      if(k.includes("late") && k.includes("hips")) add("Your transition is late in the lower body — the arms tend to win the race from the top.");
      if(k.includes("arms") && (k.includes("start") || k.includes("early"))) add("Your arms start down a touch early, which can pull the club off-plane.");
      if(k.includes("over") && k.includes("top")) add("The club wants to work outside-in (over-the-top), especially when you go after speed.");
      if(k.includes("flip") || k.includes("scoop")) add("You’re losing handle lead through impact (a little flip), which hurts compression and strike.");
      if(k.includes("early") && k.includes("extension")) add("You’re standing up through impact (early extension), which changes low point and face control.");
      if(k.includes("open") && k.includes("face")) add("The face can hang open, so you compensate late with hands or path.");
      if(k.includes("stuck")) add("You can get a bit ‘stuck’ — hands get trapped behind you, forcing late saves.");
    });

    // If we have a focus dropdown value but no good fault text, use focus as observation
    if(obs.length === 0 && focus){
      add("Based on your selected focusTopic, we’re targeting one key change that will produce the fastest payoff.");
    }

    // Build the lesson in 4 required pieces:
    // 1) What’s happening
    // 2) Why it causes the miss
    // 3) What to change
    // 4) What it should feel like (cue)
    const out: string[] = [];

    // Lead with overview if it's good (keeps it human)
    if(overview && overview.length > 12) out.push(overview);

    // 1) What’s happening
    if(obs.length){
      out.push(obs.slice(0,2).join(" "));
    } else {
      out.push("Your swing is close — the main opportunity is improving the sequencing so the body leads and the club follows.");
    }

    // 2) Why it causes the miss (plain English)
    out.push("When the sequence gets slightly out of order, you lose consistency first (strike + start line), and then you lose speed because the club is ‘thrown’ instead of delivered.");

    // 3) What to change (assignment)
    out.push(`Your assignment is simple: install “${primaryTitle}” and repeat it until the transition feels automatic.`);

    // Use up to 4 how-steps if available
    if(primaryHow.length){
      const steps = primaryHow.slice(0,4);
      out.push("How to do it:");
      steps.forEach(s => out.push("• " + s));
    }

    // Add why it works if available
    if(primaryWhy && primaryWhy.length > 10){
    ...
    // 4) Feel / cue
    if(primaryCue){
      out.push(`Feel cue: “${primaryCue}”`);
    } else {
      out.push("Feel cue: It should feel like the lower body starts first while the hands ‘finish’ the backswing for a split second.");
    }

    // Reassurance (prevents spiraling)
    out.push("Do not chase ten fixes at once. One clean change, repeated, beats five swing thoughts every time.");

    return out;
  }
  // ===== VCA_COACH_EXPLAIN_END =====
  // ===== VCA_PRIORITY_VOICE_HELPERS_START =====
  function pickPriorityText(faults: FaultKey[], focusText: string): string {
    const f = Array.isArray(faults) ? faults.join(" | ").toLowerCase() : "";
    const t = (focusText || "").toLowerCase();

    if (f.includes("over_the_top") || t.includes("slice") || t.includes("over the top") || t.includes("outside-in"))
      return "Transition first: lower body starts, arms stay soft for a split second (prevents OTT/slice).";

    if (f.includes("early_extension") || t.includes("early extend") || t.includes("stand up"))
      return "Stay in posture: push hips back + keep chest down through impact (fixes low point + contact).";

    if (f.includes("flip") || t.includes("flip") || t.includes("scoop") || t.includes("thin"))
      return "Handle leads: keep hands moving left while body rotates (stop flip, improve compression).";

    if (f.includes("open_face") || t.includes("open face") || t.includes("push"))
      return "Clubface control: improve face closure earlier so you don’t need a last-second save.";

    if (f.includes("stuck") || t.includes("stuck") || t.includes("blocked"))
      return "Create space: keep the right arm in front of you so the club can deliver from the inside without getting trapped.";

    return focusText
      ? ("Priority: " + focusText)
      : "One priority: clean sequencing (body leads, club follows).";
  }

  function voiceFeatureToast(kind: "listen" | "ask") {
    try {
      if (typeof window === "undefined") return;
      const msg = kind === "listen"
        ? "Voice playback is coming next (placeholder button)."
        : "Voice questions are coming next (placeholder button).";
      (window as any).alert?.(msg);
    } catch {}
  }
  // ===== VCA_PRIORITY_VOICE_HELPERS_END =====
  // ===== VCA_DEV_POPULATED_PREVIEW_START =====
  // Dev-only: when ?golden=1, inject a populated-looking payload so layout can be judged fairly.
  function maybeInjectGolden(data: any) {
    try {
      if (typeof window === "undefined") return data;
      const qp = new URLSearchParams(window.location.search);
      if (qp.get("golden") !== "1") return data;
      if (data?.media?.frames?.length) return data;
      ...
      const mk = (p: number) => ({
        ...
        url: "",
        thumbUrl: "",
        // minimal shape; your renderer likely uses p and/or img src
      });

      const demoFrames = [1,2,3,4,5,6,7,8,9].map(mk);
      ...
      return {
        ...
        narrative: {
          ...(data?.narrative || {}),
          overview:
            "Populated preview: This is what the page feels like with real P1–P9, scores, and drill cards filled in."
        },
        scores: {
          ...(data?.scores || {}),
          swingScore: 86,
  ...
  // ===== VCA_DEV_POPULATED_PREVIEW_END =====
  // ===== VCA_AI_HELPERS_1234_START =====
  type ConfPoint = { t: number; v: number; key: string };

  function getSwingKey(data: any): string {
    try {
      const mediaKey =
        data?.media?.framesDir ||
        data?.media?.id ||
        data?.videoUrl ||
        data?.localPath ||
        data?.media?.videoUrl ||
        "";
      if (mediaKey) return String(mediaKey);
    } catch {}
    return "vca_demo_swing";
  }
  ...
  function bumpConfHistory(key: string, value: number) {
    const k = "vca_conf_history";
    const arr = readJSON<ConfPoint[]>(k, []);
    const now = Date.now();
    const v = Math.max(0, Math.min(100, Math.round(value || 0)));

    // de-dupe: if last point for same key is within 60s and similar, overwrite
    const last = arr.length ? arr[arr.length - 1] : null;
    if (last && last.key === key && (now - last.t) < 60000 && Math.abs(last.v - v) <= 2) {
      last.t = now; last.v = v;
      writeJSON(k, arr);
      return;
    }

    arr.push({ t: now, v, key });
    // keep last 60 points total
    while (arr.length > 60) arr.shift();
    writeJSON(k, arr);
  }

  function getConfSeries(key: string): ConfPoint[] {
    const arr = readJSON<ConfPoint[]>("vca_conf_history", []);
    return arr.filter(x => x && x.key === key).slice(-12);
  }

  function inferFaultsFromData(data: any): FaultKey[] {
    const out: FaultKey[] = [];
    try {
      // common containers
      const a =
        data?.faults ||
        data?.scores?.faults ||
        data?.narrative?.faults ||
        data?.analysis?.faults ||
        [];
      if (Array.isArray(a)) {
        const s = a.map((x: any) => String(x?.key || x?.id || x || "").toLowerCase()).join(" | ");
        if (s.includes("over") && (s.includes("top") || s.includes("ott")) ) out.push("over_the_top");
        if (s.includes("early") && s.includes("extend")) out.push("early_extension");
        if (s.includes("flip") || s.includes("scoop")) out.push("flip");
        if (s.includes("open") && (s.includes("face") || s.includes("clubface"))) out.push("open_face");
        if (s.includes("sway") || s.includes("drift")) out.push("sway");
        if (s.includes("reverse") && s.includes("pivot")) out.push("reverse_pivot");
      }

      // common boolean-ish score flags
      const f = data?.scores || data?.analysis || {};
      const b = (k: string) => {
        const v = (f as any)[k];
      ...
      if (b("overTheTop") || b("ott") || b("sliceRisk")) out.push("over_the_top");
      if (b("earlyExtension") || b("goatHump")) out.push("early_extension");
      if (b("flip") || b("scoop")) out.push("flip");
      if (b("openFace") || b("faceOpen")) out.push("open_face");
      if (b("sway") || b("drift")) out.push("sway");
      if (b("reversePivot")) out.push("reverse_pivot");
    } catch {}
  ...
  function isProUnlocked(): boolean {
    try {
      ...
      const qp = new URLSearchParams(window.location.search);
      if (qp.get("pro") === "1") return true;
      return (window.localStorage.getItem("vca_access") || "") === "pro";
    } catch {
      return false;
    }
  }

  function setProUnlocked(on: boolean) {
    try {
      if (typeof window === "undefined") return;
      window.localStorage.setItem("vca_access", on ? "pro" : "demo");
    } catch {}
  }

  function readCoachNote(key: string): string {
    return readJSON<Record<string,string>>("vca_coach_notes", {})[key] || "";
  }

  function writeCoachNote(key: string, note: string) {
    const all = readJSON<Record<string,string>>("vca_coach_notes", {});
    all[key] = (note || "").slice(0, 2000);
    writeJSON("vca_coach_notes", all);
  }
  // ===== VCA_AI_HELPERS_1234_END =====
  // ===== VCA_INFER_FOCUS_START =====
  // Parse the Focus dropdown/text into a couple likely FaultKeys (best-effort).
  function inferFaultsFromFocus(focusText: string): FaultKey[] {
    const t = (focusText || "").toLowerCase();
    const out: FaultKey[] = [];

    if (t.includes("slice") || t.includes("over the top") || t.includes("outside-in") || t.includes("ott")) out.push("over_the_top");
    if (t.includes("hook") || t.includes("stuck") || t.includes("inside")) out.push("stuck");
    if (t.includes("early extend") || t.includes("stand up") || t.includes("goat")) out.push("early_extension");
    if (t.includes("flip") || t.includes("scoop") || t.includes("thin")) out.push("flip");
    if (t.includes("open face") || t.includes("face open") || t.includes("push")) out.push("open_face");
    if (t.includes("sway") || t.includes("drift")) out.push("sway");
    if (t.includes("reverse pivot")) out.push("reverse_pivot");
    if (t.includes("tempo") || t.includes("rush")) out.push("tempo");
    if (t.includes("balance")) out.push("balance");
  ...
  function computeConfidence(args: { faults: FaultKey[]; sessions: number }): { score: number; phase: string; trend: "dip" | "exit" | "climb"; note: string } {
    const vcaFaults = (args && (args as any).faults) ? (args as any).faults : [];
    const hasSeq = faults.includes("late_hips") || faults.includes("arms_start_down");
...
const [error, setError] = useState<string | null>(null);
  ...
  // ===== VCA_HAS_RESULTS =====
  const hasResults = (sortedFrames?.length ?? 0) > 0;
...
<Card title="Swing Report — Beta">
          ...
          {/* ===== VCA_PRIORITY_UI_START ===== */}
          {(() => {
            const vcaFocus = (intake?.focus || "").trim();
    const focus = vcaFocus;
            // Best-effort top faults: if you already compute faults elsewhere, use them.
            const topFaults =
              (typeof faults2 !== "undefined" && Array.isArray(faults2) ? faults2 :
               typeof faults !== "undefined" && Array.isArray(faults) ? faults :
               typeof faultsData !== "undefined" && Array.isArray(faultsData) ? faultsData :
               []) as any[];

            const summary = buildPrioritySummary({
              focusTopic,
              facts: swingFacts,
              topFaults: topFaults.map(String)
            });
              ...
              <div style={{
                ...
                <div style={{ display: "flex", justifyContent: "space-between", gap: 12, flexWrap: "wrap" }}>
                  <div style={{ fontWeight: 950, letterSpacing: 0.2 }}>
                    Priority Banner
                    <span style={{ marginLeft: 10, fontSize: 12, opacity: 0.75, fontWeight: 800 }}>
                      Confidence: {swingFacts?.confidence ?? "—"} {typeof swingFacts?.detectedRatePct === "number" ? `· ${swingFacts?.detectedRatePct}% detected` : ""}
                    </span>
                  </div>
                  ...
                  <div style={{ display: "flex", gap: 10 }}>
                    <button
                      type="button"
                      onClick={() => alert("Coming soon: voice readout of your summary.")}
                      style={{
                        height: 36,
                        padding: "0 12px",
                        borderRadius: 12,
                    ...
                    <button
                      type="button"
                      onClick={() => alert("Coming soon: ask by voice + get answers.")}
                      style={{
                        height: 36,
                        padding: "0 12px",
                        borderRadius: 12,
                ...
                <div style={{ marginTop: 10, display: "flex", gap: 10, flexWrap: "wrap", fontSize: 12, opacity: 0.85 }}>
                  <span style={{ padding: "2px 10px", borderRadius: 999, border: "1px solid rgba(255,255,255,0.14)" }}>
                    Tempo: <b>{swingFacts?.tempo?.backswingSec ?? "—"}</b>s / <b>{swingFacts?.tempo?.downswingSec ?? "—"}</b>s
                  </span>
                  <span style={{ padding: "2px 10px", borderRadius: 999, border: "1px solid rgba(255,255,255,0.14)" }}>
                    P4(top): <b>{swingFacts?.p?.P4 ?? "—"}</b> · P7(impact): <b>{swingFacts?.p?.P7 ?? "—"}</b>
                  </span>
                </div>
              </div>
            );
...
{/* ===== VCA_COACH_UI_START ===== */}
          {(() => {
            // Source inputs (best-effort; nothing here should crash)
            const focusText = ((intake as any)?.focus || (typeof focusTopic === "string" ? focusTopic : "") || "").toString();
            const overview = (data as any)?.narrative?.overview || (data as any)?.narrative?.summary || "";

            // Build faults safely
            let faultsFromFocus: FaultKey[] = [];
            try { faultsFromFocus = (typeof inferFaultsFromFocus === "function") ? (inferFaultsFromFocus(focusText) as any) : []; } catch { faultsFromFocus = []; }

            let faultsFromData: FaultKey[] = [];
            try { faultsFromData = (typeof inferFaultsFromData === "function") ? (inferFaultsFromData(data) as any) : []; } catch { faultsFromData = []; }

            const faultsLesson = Array.from(new Set([...(faultsFromData || []), ...(faultsFromFocus || [])])).slice(0, 2) as FaultKey[];

            
            const priorityText = pickPriorityText(faultsLesson, focusText);
            const __vcaPriorityBanner = true; // VCA_PRIORITY_BANNER_UI
// Build drill Rx (ok if empty)
            let rx: any[] = [];
            try {
              rx = (typeof prescribe === "function")
                ? (prescribe({ faults: faultsLesson, level: (level as any) ?? "intermediate", junior: (intake as any)?.audience === "junior" }) as any)
                : [];
            } catch { rx = []; }

            const lines = buildCoachExplanation({ focusText, faults: faultsLesson, rx, overview });

            return (
              <TinyCard title="Swing Summary (Coach Explanation)">
                
                {/* ===== VCA_PRIORITY_BANNER_UI_START ===== */}
                <div style={{
                  display: "flex",
                  gap: 10,
                  alignItems: "center",
                  ...
                  <div style={{ fontSize: 13, fontWeight: 900, letterSpacing: 0.2 }}>
                    PRIORITY: <span style={{ fontWeight: 800, opacity: 0.92 }}>{priorityText}</span>
                  </div>
                  <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
                    <button
                      onClick={() => voiceFeatureToast("listen")}
                      style={{
                        height: 30,
                        padding: "0 10px",
                        borderRadius: 10,
                      ...
                      title="Listen to coach explanation (coming soon)"
                    >
                      🔊 Listen
                    </button>
                    <button
                      onClick={() => voiceFeatureToast("ask")}
                      style={{
                        height: 30,
                        padding: "0 10px",
                        borderRadius: 10,
                      ...
                      title="Ask a question by voice (coming soon)"
                    >
                      🎙️ Ask by Voice
                    </button>
                  </div>
                </div>
                {/* ===== VCA_PRIORITY_BANNER_UI_END ===== */}
<div style={{ fontSize: 13, lineHeight: 1.55, opacity: 0.95 }}>
                  {lines.map((t, i) => (
                    <div key={i} style={{ marginTop: i === 0 ? 0 : 8 }}>
                      {t.startsWith("•") ? <span>{t}</span> : <span>{t}</span>}
                    </div>
                  ))}
                </div>
              </TinyCard>
          ...
          {hasResults && (
            <>
          {(() => {
            const vcaFocus2 = (intake?.focus || "").trim();
            const focusText = vcaFocus2;
            const vcaFaults2 = (typeof inferFaultsFromFocus === "function"
              ? (inferFaultsFromFocus(focusText || (typeof focusTopic === "string" ? focusTopic : "")) as any)
              : []) as FaultKey[];

            const faults2 = vcaFaults2;
            const faults2 = vcaFaults2;
            const faults2 = vcaFaults2;
...
const sessions = (typeof window !== "undefined" && (window as any).__vca_sessions) ? (window as any).__vca_sessions : 0;
            ...
            // ===== VCA_CONF_HISTORY_WRITE =====
            const swingKey = getSwingKey(data);
            const pro = isProUnlocked();

            const series = getConfSeries(swingKey);
            const rx = prescribe({ faults2: faults2, level: (intake?.level as any) || "intermediate", junior: intake?.audience === "junior" });
            const picks = (rx?.picks || []).slice(0, pro ? 2 : 1);
            // ===== VCA_CONF_HISTORY_WRITE_END =====
return (
              <div style={{ marginTop: 14, display: "grid", gridTemplateColumns: "1fr 1fr", gap: 14 }}>
                ...
                {/* ===== VCA_PRO_UNLOCK_UI_START ===== */}
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginTop: 10, gap: 10 }}>
                  <div style={{ display: "flex", gap: 6, alignItems: "center" }}>
                    <div style={{ fontSize: 11, opacity: 0.75 }}>History</div>
                    <div style={{ display: "flex", gap: 4, alignItems: "flex-end", height: 18 }}>
                      {(series || []).map((pt, i) => (
                        <div key={i} style={{ width: 8, height: Math.max(4, Math.round((pt.v || 0) / 6)), borderRadius: 6, background: "rgba(255,255,255,0.35)" }} />
                      ))}
                    </div>
                  </div>
                  <button
                    onClick={() => { setProUnlocked(!pro); try { location.reload(); } catch {} }}
                    style={{
                      height: 30,
                      padding: "0 10px",
                      borderRadius: 10,
                    ...
                    title="Toggle demo/pro locally (no server)."
                  >
                    {pro ? "Pro: ON" : "Pro: OFF"}
                  </button>
                </div>
                {/* ===== VCA_PRO_UNLOCK_UI_END ===== */}
