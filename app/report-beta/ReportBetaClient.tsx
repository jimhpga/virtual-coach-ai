"use client";
  const clipUrl = (report as any)?.clipUrl || (report as any)?.clip?.url || null;
...
{clipUrl ? (
  <div id="vca-video-window" style={{ margin: "10px 0 14px 0", borderRadius: 14, overflow: "hidden", border: "1px solid rgba(255,255,255,0.10)", background: "rgba(0,0,0,0.22)" }}>
    <div style={{ padding: "10px 12px", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
      <div style={{ fontSize: 12, fontWeight: 800, opacity: 0.92 }}>Swing clip</div>
      <a href={clipUrl} target="_blank" rel="noreferrer" style={{ fontSize: 12, opacity: 0.8, textDecoration: "underline" }}>Open MP4</a>
    </div>
    <video src={clipUrl} controls playsInline preload="metadata" style={{ width: "100%", height: "auto", display: "block" }} />
  </div>
) : null}
    PLAYER OVERVIEW
  </div>
