...
const [data, setData] = React.useState<FullData>(FALLBACK);
  ...
  React.useEffect(() => {
    ...
    src &&   // ---- Source routing (src preferred; jobId fallback) ----
  const qs = (typeof window !== "undefined") ? new URLSearchParams(window.location.search) : null;
  const srcQ = qs?.get("src") || "";
  const jobIdQ = qs?.get("jobId") || "";
  // Primary: direct JSON URL (rep.url). Fallback: try local API by jobId if present.
  const resolvedSrc = srcQ || (jobIdQ ? (`/api/report?id=${encodeURIComponent(jobIdQ)}`) : "");
