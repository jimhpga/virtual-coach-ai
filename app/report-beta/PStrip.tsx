  ...
  return (
    <div style={{ marginTop: 12 }}>
      <div style={{ display:"flex", gap:8, flexWrap:"wrap", alignItems:"center", marginBottom: 8 }}>
        <div style={{ fontWeight: 800, opacity: 0.9 }}>Phases:</div>
        {frames.map(f => (
          <button
            key={f.p}
            onClick={() => pick(f.p)}
            ...
            title={`Go to ${`Phase ${f.p}`} (frame ${f.frame})`}
          >
            {`Phase ${f.p}`}
          </button>
        ))}
        ...
        {frames.map(f => (
          ...
          >
            ...
            <div style={{ padding:"6px 8px", fontSize:12, opacity:0.9, display:"flex", justifyContent:"space-between" }}>
              <span style={{ fontWeight: 800 }}>{`Phase ${f.p}`}</span>
