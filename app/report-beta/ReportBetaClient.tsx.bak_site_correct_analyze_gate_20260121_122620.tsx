...
<div style={{ display: "grid", gap: 12 }}>
    ...
    <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12 }}>
      ...
      >
        ...
        {runState === "frames" && "Processing frames…"}
        {runState === "report" && "Building report…"}
        {runState === "error" && "Try again"}
      </button>

      {runState !== "idle" && (
        <div style={{ fontSize: 12, opacity: 0.75, marginTop: 6 }}>
          {runState === "analyzing" && "Running swing analysis…"}
          {runState === "frames" && "Extracting key positions (P1–P9)…"}
          {runState === "report" && "Generating your report…"}
          {runState === "error" && runErr}
        </div>
      )}
